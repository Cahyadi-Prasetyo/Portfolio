...
export default function Window({ id, title, children, initialPosition = { x: 100, y: 50 }, initialSize = { width: 'min(800px, 90vw)', height: 'min(600px, 80vh)' } }: WindowProps) {
    const { windows, closeApp, focusWindow, minimizeWindow, toggleMaximizeWindow, activeWindowId } = useWindowManager();
    ...
    // Auto-maximize on mobile, or use state
            ...
            {windowState.isOpen && !windowState.isMinimized && (
                <motion.div
                    ...
                    animate={{
                        ...
                        height: isMaximized ? 'calc(100vh - 32px)' : initialSize.height,
                    ...
                    className={`absolute overflow-hidden shadow-2xl border border-white/10 bg-gray-900/60 backdrop-blur-2xl flex flex-col min-w-[300px] min-h-[200px] ring-1 ring-white/5 ${!isMaximized ? 'rounded-xl' : ''}`}
                    style={{
                        // Width/Height handled by animate prop when maximized
                ...
                >
                    ...
                    {/* Title Bar */}
                    <div
                        className={`h-11 flex items-center justify-between px-4 select-none ${!isMaximized ? 'cursor-grab active:cursor-grabbing' : ''} border-b border-white/5 relative z-10 transition-colors duration-200 ${isActive ? 'bg-white/5' : 'bg-transparent'}`}
                    ...
                    >
                        <div className="flex items-center gap-2">
                            ...
                            <button onClick={(e) => { e.stopPropagation(); toggleMaximizeWindow(id); }} className="w-3 h-3 rounded-full bg-green-500 hover:bg-green-600 flex items-center justify-center group">
